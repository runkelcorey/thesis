...
`r quote_footer("--William Levitt, 1948")`
...
Summary statistics for the complete data are available in the following table.

``` {r summary, echo=FALSE, results='asis', eval=TRUE}
stargazer(as.data.frame(wide), type = "html", header = F, keep = c(7:11,13,14,16:18,28,35,37,41:45,47,49), summary.logical = T, summary.stat = c("mean", "sd", "min", "median", "max"), title = "Descriptive statistics", covariate.labels = c("White \\%", "Hispanic \\%", "Black \\%", "Asian \\%", "65-and-up \\%", "Some college or less \\%", "Median HHI (000s)", "Density", "Evangelical \\%", "L.D.S. \\%", "Population", "Republican voting \\%", "Miles to NSP1", "Mortgage occupancy \\%", "Foreclosure rate", "Labor variance", "Unemployment change", "Price change '07--'10", "Tenure in home", "Cook PVI"), label = "tab:summary")
```
...
```{r hierarchy, fig.margin=TRUE, fig.cap="Organizational hierarchy of different geographies.", echo=FALSE, fig.width=3.5}
include_graphics("figure/geo_hierarchy.png")
```
...
See Figure \@ref(fig:hierarchy) for a graphical representation.[@lemery2020united]

``` {r geography, fig.cap="Relationships between different statistical entities", fig.margin=TRUE, echo=FALSE, fig.width=3.5}
include_graphics("figure/census_small_area_geography2.jpg")
```
...
Figure \@ref(fig:geography) shows the Census Bureau's graphical representation of geographical relationships.
...
```{r palmbeach, fig.margin=TRUE, fig.cap=c("Voting precincts and census tracts in Palm Beach County, Florida without census tracts...", "...and with census tracts."), echo=FALSE}
...
\end{equation}
Where *t* represents a particular tract, *p* a precinct from the set *P* of precincts intersecting with that particular tract, and *b* a block from the set *B* of blocks within that particular precinct:
```
...
This method is represented formally to the right.
...
The following table lists the effects of NSP1 on prices.
...
stargazer(mod1, mod2, mod3, type = "html", title = "Effect of NSP1 on Prices", omit = c("^STATE", "FORQ2:MORTOCC", "MORTOCC:FORQ3"), label = "tab:prices", df = F, align = T, report = "vc*", header = F, add.lines = list(c("State fixed Effects?", "Y", "N", "Y")), digits = 4, no.space = T, covariate.labels = c(NA, "Labor risk", "2010 unemployment rate", "Median household income", "2009 foreclosure rate", "Mortgaged occupancy rate", "2010 forelcosure rate", "Population density", "65-and-up \\%", "Tenure in home", "Squared tenure", "White \\%", "Black \\%", "Hispanic \\%", "Asian \\%", "Unemployment change '05--'10", "High-cost mortgage rate", "Seriously delinquent rate"), dep.var.labels = "Home price index change, 2007-2010")
```

```{r prices, echo=FALSE, fig.margin=TRUE, fig.cap=c("Residuals on fitted values from price estimation", "Sample quantiles on theoretical quantiles.")}
df <- augment(mod1)
d <- ggplot(df, aes(x = .fitted, y = .resid)) +
  geom_point() +
  theme_tufte(ticks = F) +
  theme(axis.title=element_blank(), axis.text=element_blank())
ggMarginal(d, type = "density")
...
The next table lists the results of the full-sample models with and without accounting for prices.
...
mod5 <- lm(RPCT ~ NSP1 + ADJ_FORQ*ADJ_PRICE07 + EMPLOYRISK*ADJ_PRICE07 + UNEMCHG + MEDHHI + ADJ_FORQ*DENSITY + TENURE + TENURE2 + OLD + VAP_B + VAP_H + VAP_H2 + VAP_A + VAP_W*WHITESOMECOL + EVANRATE + LDSRATE + PVI08, wide)
stargazer(mod4, mod5, type = "html", title = "Linear regression of NSP1 on Voting", label = "tab:fullsample", df = F, align = T, report = "vc*", header = F, digits = 4, no.space = T, covariate.labels = c(NA, "2010 forelcosure rate", "Home price change '07--'10", "Labor risk", "Unemployment change '05--'10", "Median household income (000s)", "Population density", "Tenure in home", "Squared tenure", "65-and-up \\%", "Black \\%", "Hispanic \\%", "Hispanic \\% squared", "Asian \\%", "White \\%", "White: some college or less \\%", "Evangelical \\%", "L.D.S. \\%", "Cook PVI 2008", "Price change $\\times$ foreclosure rate", "Price change $\\times$ labor risk", "Foreclosure rate $\\times$ density", "White \\% $\\times$ some college or less"), dep.var.labels = "Republican vote percentage", model.numbers = F, column.labels = c("(4)", "(5)"))
```
...
While there are reasons *not* to accept this model's validity, the consistency of the expectations with the model---and prior literature---is a reason to believe its capacity to describe impacts of economic and demographic variables on majoritarian voting behavior.
...
I account for it with post-crisis literature which argues that individuals with risky occupations face additional barriers to securing home equity lines of credit (HELOCs).[@mota2015spatial, Table 1]
...
The following shows the results of the same model above, but applied only to tracts in which a self-described member of the Tea Party ran for election.
...
mod7 <- lm(RPCT ~ NSP1 + ADJ_FORQ*ADJ_PRICE07 + EMPLOYRISK*ADJ_PRICE07 + UNEMCHG + MEDHHI + ADJ_FORQ*DENSITY + TENURE + TENURE2 + OLD + VAP_B + VAP_H + VAP_H2 + VAP_A + VAP_W*WHITESOMECOL + EVANRATE + LDSRATE + PVI08, filter(wide, RESULT != "EXTRA-TP"))
stargazer(mod6, mod7, type = "html", title = "Linear regression of NSP1 on Voting", label = "tab:justp", df = F, align = T, report = "vc*", header = F, digits = 4, no.space = T, covariate.labels = c(NA, "2010 forelcosure rate", "Home price change '07--'10", "Labor risk", "Unemployment change '05--'10", "Median household income (000s)", "Population density", "Tenure in home", "Squared tenure", "65-and-up \\%", "Black \\%", "Hispanic \\%", "Hispanic \\% squared", "Asian \\%", "White \\%", "White: some college or less \\%", "Evangelical \\%", "L.D.S. \\%", "Cook PVI 2008", "Price change $\\times$ foreclosure rate", "Price change $\\times$ labor risk", "Foreclosure rate $\\times$ density", "White \\% $\\times$ some college or less"), dep.var.labels = "Republican vote percentage", model.numbers = F, column.labels = c("(6)", "(7)"))
```

These trials offer promising results but less statistical significance.^[The [unprinted] p-value attached to Model 6 is just north of .10, nearly acceptable among social scientists.]
...
A comparison of how each of the NSP rounds fared with respect to Republican votes is available in the following table.
...
```{r compnsp, results='asis', echo=FALSE}
...
``` {r linear, echo=FALSE, fig.margin=TRUE, fig.cap="Diagnostic plots of the full linear model.", fig.show='hold'}
df <- augment(mod5)
d <- ggplot(df, aes(x = .fitted, y = .resid)) +
  geom_point() +
  theme_tufte(ticks = F) +
...
Figure \@ref(fig:linear) shows the residuals against fitted values for the full-sample, fixed-effects model, as well as the quantile-quantile distribution of residuals.
...
I settled on reporting results for regressions on the Beta distribution because of its flexibility[@cribarineto2010beta] and ability to approximate the distribution of Republican voting share.
...
``` {r betaresid, echo=FALSE, fig.margin=TRUE, fig.cap="Standardized weighted residuals of full beta regression.", message=FALSE}
...
Results of the beta regressions can be seen in the next table.
...
mod9 <- betareg(ADJ_RPCT ~ NSP1 + ADJ_FORQ*ADJ_PRICE07 + EMPLOYRISK*ADJ_PRICE07 + UNEMCHG + MEDHHI + ADJ_FORQ*DENSITY + TENURE + TENURE2 + OLD + VAP_B + VAP_H + VAP_H2 + VAP_W*WHITESOMECOL + EVANRATE + LDSRATE + PVI08, mutate_at(wide, vars(ADJ_PRICE07, ADJ_FORQ, EMPLOYRISK, UNEMCHG, MEDHHI, DENSITY, TENURE, TENURE2, OLD, VAP_B, VAP_H, VAP_H2, VAP_A, VAP_W, WHITESOMECOL, EVANRATE, LDSRATE), funs(./100)))
stargazer(mod8, mod9, type = "html", title = "Beta regression of NSP1 on Voting", label = "tab:beta", df = F, align = T, report = "vc*", header = F, digits = 4, no.space = T, covariate.labels = c(NA, "2010 forelcosure rate", "Home price change '07--'10", "Labor risk", "Unemployment change '05--'10", "Median household income (000s)", "Population density", "Tenure in home", "Squared tenure", "65-and-up \\%", "Black \\%", "Hispanic \\%", "Hispanic \\% squared", "Asian \\%", "White \\%", "White: some college or less \\%", "Evangelical \\%", "L.D.S. \\%", "Cook PVI 2008", "Price change $\\times$ foreclosure rate", "Price change $\\times$ labor risk", "Foreclosure rate $\\times$ density", "White \\% $\\times$ some college or less"), dep.var.labels = "Republican vote percentage", model.numbers = F, column.labels = c("(8)", "(9)"))
```
...
Nonetheless, I can prove the presence of spatial dependence in my models to highlight future areas for exploration.[@bivand2018comparing]
...
kable(spdep, "html", booktabs = T, caption = "Results of various tests for spatial dependence. In all tests, the null hypothesis is spatial independence.", align = 'c', digits = 4)
```
