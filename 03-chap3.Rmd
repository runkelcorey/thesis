...
Therefore, while the motives for voting Republican by Hispanics are complex, the reasons do not cancel out one another.

On the other hand, Asian Americans have pushed back against the "Model Minority" myth to contextualize the fact that Asian Americans---come their families (or they) from India, China, Vietnam, Japan, Korea, Iran, Uzbekistan, or the Philippines---make on average $Z,ZZZ more than the average white household.
...
Republican voting by evangelical Protestants and members of the Church of Latter-Day Saints (LDS, or the Mormon Church) is much more predictable than voting by members adherent to any religion.
...
Lastly, the highest level of education attained is a huge predictor of one's party affiliation.
Pew Research has tracked party identification by education level, indicating that the white vote cleaves once introducing education level.
Pursuant to this fact, I will puruse both education level multiplied by percentage of population white as a predictor.

## A Model of Voting {#model}

The discussion above leaves me with a model of Republican voting:
$rep_i = \beta_0 + \beta_1forq_i + \beta_2Black_i  + \beta_3AmerIndian_i + \beta_4urbanicity_i + \beta_5evang_i + \beta_6lds_i + \beta_7Hispanic_i + \beta_8medHHI_i + \beta_9chgHHI_i + \beta_{10}ownhome_i + \beta_{11}unem10_i + \beta_{12}old_i + \beta_{13}(somecol_i \times white_i) + \epsilon$