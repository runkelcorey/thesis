...
library(sf)
library(betareg)
options(tigris_class = "sf", scipen = 9999, digits = 2)
...
#kable(read_csv("data/voting/display/tract_coverage.csv"))
```
...
```{r geography, out.width='45%', fig.align='center', fig.cap="Left: To-scale breakdown of nested census geography, with street names. Right: Organizational hierarchy of different geographies.", echo=FALSE}
...
stargazer(mod1, mod2, type = "latex", title = "Full-sample linear models", omit = "^STATE", label = "tab:fullsample", font.size = "small", column.sep.width = "3pt", df = F) #write table output
```
...
stargazer(mod3, mod4, mod5, mod6, type = "latex", title = "Tea-Party linear models", omit = "^STATE", label = "tab:justp", font.size = "small", column.sep.width = "3pt", df = F)
```
...
``` {r linear, echo=FALSE, out.width='45%', fig.align='center', fig.cap="Left: Residuals of full linear model. Right: Acutal vs. fitted quantiles of full linear model."}
...
stargazer(mod7, mod8, mod9, type = "latex", title = "Beta regressions", omit = "^STATE", label = "tab:beta", font.size = "small", column.sep.width = "3pt", df = FALSE)
```
