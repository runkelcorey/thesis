...
### Voting Impacts of the NSP and Price Change {#impacts}
...
But doing so would reject the other results of this model that conform to theoretical expecations as well as prior research.
...
Both the triple interaction between prices, foreclosures, and labor risk, as well as the price change--foreclosure rate interaction hold positve coefficients, consistent with the model's prediction that low-equity and high-labor risk combine to decrease preferences for social insurance, tied in the United States to higher taxation.
...
I account for this with post-crisis literature which additional difficulties associted with securing home equity lines of credit (HELOCs) for individuals with risky occupations.[@mota2015spatial, Table 1]
...
Nonetheless, even if this phenomena is not occurring, the labor risk--equity model and prior literature on Republican voting behavior agree elsewhere with my regression results.

On the other hand, one could accept the model's capacity, opening up the possibility that the NSP's primary political effect is not in its home equity impacts.
...
mod6 <- lm(RPCT ~ NSP1 + ANXIETY + ADJ_FORQ + EMPLOYRISK + UNEM10 + MEDHHI + ADJ_FORQ*DENSITY + TENURE + TENURE2 + OLD + VAP_B + VAP_H + VAP_H2 + VAP_W*WHITESOMECOL + EVANRATE + LDSRATE, filter(wide, RESULT != "EXTRA-TP"))
