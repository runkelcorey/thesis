...
`r quote_footer("George W. Bush, October 2004")`
...
A product of contentious debate on the House floor (the bill faced 17 different roll call votes on its route to the President's desk[@pelosi2008housing]), HERA reflected a cacophony of political voices.
While federal legislation is more a product of compromise than parliamentary legislation, American laws can be decomposed into smaller titles and programs that closer reflect the wills of particular caucuses or politicians.^[It is this by this characteristic that individual amendments are labeled as "pork-barrel politics".]
...
Both Republican and Democratic parties have connected programs that transfer taxpayer money to less affluent people with higher taxation, and with the post--New Deal Democratic Party.[@schwartz2009politics, 212]
...
A select few news sources picked up on the insertion, including a Reuters piece which claimed "The White House had originally opposed a provision that offers $4 billion in grants to states to buy and repair foreclosed homes."[@pelofsky2008bush]
...
By mitigating spillover effects, the Neighborhood Stabilization Program would cosset homes from the "animal spirits"[@keynes2007general] of tumultuous housing markets and ensure that the success or failure of a person's finances depended on their own actions.

```{r distress, fig.cap="Probability of home loss by owner type. The lag between subprime and prime was partially because there was more 'fuel left to burn' in the sense that the number of subprime borrowers, already only 20% of the market, had already been slashed.", echo = FALSE, fig.margin=TRUE}
include_graphics( "figure/ferreira_distress.png")
```
...
The necessity of these measures is questionable: while 45% of Sand State mortgages issued in 2006 were for second, third, or fourth properties,[@mian2011house, 2150] it is important to remember that the majority of those foreclosed upon were prime borrowers.
Further, the distress of prime borrowers came somewhat after that of subprime borrowers, as seen in Figure \@ref(fig:distress), taken from @ferreira2015new.
...
Or consider the mortgage interest deduction, a large subsidy applied to owner-occupied properties often considered part of the "hidden welfare state".[@howard1997hidden]
...
The 9% drop in Democratic share of Latinx voting between 2000 and 2004 may be partially attributable to the 2.8% spike in homeownership in that time.[@schwartz2018politics]
But since all this movement occurred at the level of the executive, and the implications of easy mortgage credit seemed not to be picked up by politicians outside Florida, a distance sustained between Republican rhetoric and the *de facto* party policy led by the Bush administration.
...
These included passage of the Garn--St. Germain Depository Institutions Act of 1982---which Paul Krugman held responsible for the savings and loan crisis (if not for the macroeconomic factors that initially stressed S&Ls) by granting permission to private lenders for the sale of adjustable-rate mortgages[@krugman2009reagan]---but I pay particular attention to the exaltation of multinational creditors in mortgage markets.
...
```{r hhdebt, out.width=10, echo=FALSE, fig.fullwidth=TRUE, fig.cap="Outstanding US residential mortgage debt as a percentage of US gross domestic product."}
...
```{r minsky, fig.margin=TRUE, fig.cap="A cycle of speculative finance theorized by Hyman Minsky.", echo=FALSE, dev='pdf'}
...
There have *always* been large chunks of discretionary spending that could be trimmed.
...
```{r acceptance, fig.cap="Number of tax mentions by party in presidential nomination acceptance speeches.", fig.margin=TRUE, echo=FALSE}
include_graphics("figure/campbell.png")
```

```{r ads, fig.cap="Percentage of general election TV ads to feature taxation, by party.", echo=FALSE, fig.margin=TRUE}
include_graphics("figure/campbell2.png")
```

After the reforms of the first term, the campaign ramped up its tax rhetoric.
Figure \@ref(fig:acceptance) shows the massive spike in tax mentions in nomination acceptance speeches.
Figure \@ref(fig:ads) meanwhile shows a similar spike of mentions in general election television ads aligning with the 1984 election.
...
```{r publicdebt, fig.fullwidth=TRUE, fig.width=10, fig.cap="Federal public debt as a percentage of gross domestic product.", echo=FALSE, dev='pdf'}
...
```{r deficit, fig.margin=TRUE, fig.width=2.5, fig.cap="Percentage of respondents who say that reducing the budget deifict is a top priority by party affiliation.", echo=FALSE, dev='pdf'}
...
In addition to buttressing my methodology, this chapter described the anti-tax politics that changed a nation of pliant post-War taxpayers[@campbell2009how, 160] into one of activists.
Responses to California's Proposition 13 and reverberations of Reaganite rhetoric led Republicans "to define themselves and their party in opposition to taxes."[@martin2008welcome, 128]
