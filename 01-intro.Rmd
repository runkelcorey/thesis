...
```{r cc, echo=FALSE, fig.margin=TRUE, fig.show='hold'}
include_graphics(c("https://mirrors.creativecommons.org/presskit/icons/cc.xlarge.png", "https://mirrors.creativecommons.org/presskit/icons/by.xlarge.png", "https://mirrors.creativecommons.org/presskit/icons/sa.xlarge.png"))
```
...
To that end, I have provided Figure \@ref(fig:analysisfig).
...
```{r analysis, echo=FALSE, eval=FALSE, fig.cap="Diagram of the argument, by chapter: gray = \\@ref(actors-motive); tan = \\@ref(motive-opportunity); orange = \\@ref(methods); red = \\@ref(outcome)."}
DiagrammeR::mermaid("graph TB; A[Proposition 13]-->|Tax cutting|C{Reagan<br>admin.};
...
```{r analysisfig, echo=FALSE, eval=TRUE, out.width='70%', fig.align='center', fig.cap="Diagram of the argument, by chapter: gray = \\ref{actors-motive}; tan = \\ref{motive-opportunity}; orange = \\ref{methods}; red = \\ref{outcome}."}
...
Mortgages originated in 2006 and 2007 respectively suffered default rates double and quadruple the rates of those originated in 2004,[@schwartz2009politics, 200] with many of them clustering in the Sand States of California, Nevada, Arizona, and Florida.
Countrywide Financial, which two years earlier had serviced 20% of US mortgages,[@schwartz2009politics] collapsed in July 2008 (though it was subsequently resurrected under Bank of America).
...
Instead, Congress passed the Housing and Economic Recovery Act of 2008 (HERA). It authorized the Department of Housing and Urban Development (HUD) to distribute $3.9 billion to state and local governments for the purchase and repair of foreclosed properties.[@pelosi2008housing]
...
On top of this comes the stigma of being a "deadbeat"[@dayenChainTitleHow2016] debtor and mounds of fees---legal, cancellation, transportation-related.
But once homeowners have been forced out, the neighborhood and municipality bear the financial weight of that foreclosure.

To nearby homeowners, foreclosures cost on average $159,000 in decreased property values.[@immergluck2011foreclosed, 151]
...
``` {r caseshiller, fig.cap = "Price (right axis) and growth (left axis) of the Case-Shiller Home Price Index, adjusted for seasonal price fluctuations.", echo = FALSE, fig.fullwidth = TRUE, fig.width=10}
...
However, farmers in the Great Depression differed from early 2000s subprime borrowers in that farming did not *supplement* wage incomes, it *replaced* wage incomes.
In addition, access to government and stark differences in media portrayal combined with the larger impact of farm foreclosures to drive organizing that led to 27 states enacting *per se* or *de facto* moratoria on foreclosures.[@wheelock2008changing, 537]
...
In 1933, by far the worst year for farm mortgages, failure rates topped 3.7%; during the rest of 1926-1940, rates were often above 1.5%.[@wheelock2008changing, 571]
...
```{r wheelock-farms, fig.cap="Spatial clustering of farm foreclosures. From Wheelock (2008).", echo=FALSE, fig.margin=TRUE}
...
While farms produce a variety of crops, soil and climate particularities combine with federal agriculture policy to homogenize production locally.[@pollan2006omnivore]
...
For both populations, spatial correlations were imperfect, as some farmers planted different crops and some foreclosures occurred among conservative borrowers or wholly-owned homes, but the spillover effect mattered.[@defusco2018role]
...
As an indication of how weak HOAs were, homeowner association fees were some of the first payments to stop once mortgage debt piled up.[@perkins2010privatopia, 561]
...
```{r tradingecon, fig.cap = "Existing Home Sales versus New Home Sales, 2000-2014.", echo=FALSE, fig.margin=TRUE}
...
@collins2011state found that even such vanilla advocacy as mailings suggesting loan modification were more effective in states with judicial foreclosures.
However, evidence regarding the change of this process over time is a mixed bag.
@cheung2014homeowners argue that the rights of residents have eroded as the number of states with non-judicial foreclosures are the norm has increased.^[Though the actual number of states where non-judicial foreclosure is legally available has decreased, per @ghent2012historical, 22-23.]
...
Allied state attorneys general settled with 13 banks over their roles in using falsified titles, signatures, and documents to foreclose on 3.8 million borrowers.[@orol2013breaks]
...
But during the mortgage crisis, calls for mortgage moratoria were met with consternation, as scholarly opinion soured on *Blaisdell*.
...
While scholarship has elaborated (and debated) Charles Beard's story of private interests in the Constitutional Convention, *An Economic Interpretation of the U.S. Constitution* is still valuable for its analysis of primary sources.
...
Richard Epstein, one of the Chicago School's leading legal scholars, wrote that "the police power exception has come to eviscerate the contracts clause,"[@epstein1984revitalization, 738] and he is not alone.
Indeed, Tim Geithner, Treasury Secretary under President Barack Obama, made sure to emphasize legality as one of the tenets of his crisis responses, and pointed to state usage of the Contract Clause as a grey area.[@mcnamara2019yale]
...
For instance, while *Blaisdell* squeaked by with a 5-4 decision, *Wickard v. Filburn* unanimously upheld the authority of the Agricultural Adjustment Act to regulate private acts---ones that had never seen a marketplace---which substantially effected the business of another state.[@1942wickard]
*Wickard* capped five to seven years of the Supreme Court leveraging the Commerce Clause's to authorize New Deal policies.
...
```{r spending-shift, fig.cap = "Government spending and revenue as a percentage of GDP. While secular growth is present at all levels of government, note the spikes during the Great Depression, Recession, and wars.", fig.fullwidth=TRUE, echo=FALSE}
include_graphics("figure/spending-shift.png")
```

As federal responsibility grew and local responsibility concomitantly shrunk, homeowners began throttling property taxes, which account for 70% of municipal revenues.[@schwartz2009subprime, 181]
...
Endowed only with powers to issue zoning regulations, tax *incentives*, and other growth-oriented policies,[@stein2019capital] cities and states were less able to handle the wave of foreclosures that forced one of every twenty American adults out of their home.
...
```{r immergluck, fig.align='center', fig.cap="Organization of securitized mortgage lending. Taken from Immergluck (2011).", echo=FALSE, dev='pdf', out.width='100%'}
...
During the Great Depression, Prudential, then the largest holder of farm debt in the country, voluntarily ceased collection on farm mortgages.[@fliter2012fighting, 65]
...
It is likely that the historical reality of American residential mortgage-backed security would have prevented this anyway: foreign capital held 20% of residential MBS in the United States.[@schwartz2009subprime]
